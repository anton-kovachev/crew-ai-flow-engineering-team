...
- When using `write_sandbox_file`, do not call it until the entire target file content is already complete in memory. The tool call must include both `relative_path` and `content` together; never call it with only `relative_path` or with a placeholder body.
- Do not install or use any additional third-party libraries unless they are explicitly mentioned in the prompt or already listed as project dependencies; stick to the allowed set for the task.
- Keep implementations minimal and requirement-driven: do not add extra features, abstractions, or behavior that are not explicitly required by the prompt.
